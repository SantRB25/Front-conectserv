  ...
  callbacks: {
    ...
    async jwt({ token, user, account, profile }) {
      ...
      if (account?.provider === "google" && profile?.sub) {
        try {
          console.log("Google login: Intentando login con medio: google, id:", profile.sub)

          // Llamar directamente a login sin verificación previa
          const loginResponse = await api.professional.login("google", profile.sub, "")

          console.log("Google login: Respuesta de login:", loginResponse)
          ...
          if (loginResponse.success && loginResponse.data) {
            ...
            // No es necesario hacer nada más, la sesión se creará automáticamente
            console.log("Google login: Autenticación exitosa")
          } else {
            // Si el login falla, rechazar la autenticación
            console.log("Google login: Autenticación fallida")
            return null as any
          }
        } catch (error) {
